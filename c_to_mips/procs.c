...
void print_sprite(int charcode)
{
	...
	background_color(attr.color - 100);
...
void accel_process_arrows()
{
	...
	if (n < 2)
		return; // Not an arrow key sequence
	if (buffer[0] != 91)
		return; // Not an arrow key sequence
	if ((buffer[1] < 65) || (buffer[1] > 68))
		return;											// Not an arrow key sequence
...
int get_key()
{
	...
	{ // iterate through key_array to find match
		int k = key_array[i];
		if (k == ch)
			return 1 + i;
	}
...
int note_intervals[] = {
		// These are *halfnotes* above the actual notes, i.e., midpoints between consecutive notes
...
void audio_callback(void *userdata, Uint8 *stream, int len)
{
	...
	for (int i = 0; i < samples; i++)
	{
		...
		current_phase += 2.0f * 3.14159267 * frequency / SAMPLE_RATE;

		if (current_phase > 2.0f * 3.14159267)
		{
			current_phase -= 2.0f * 3.14159267;
		}
	}
}
